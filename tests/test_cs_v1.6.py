...
class ParameterOverlay:
    ...
    def keep_topmost(self, force=False):
        ...
        # 普通帧只把窗口维持在topmost层；定期先移出再重新插入，
        # 从而压到其他topmost窗口之上，同时使用NOACTIVATE避免抢焦点。
        if (
            force
            or now - self.last_topmost_refresh
...
class DirectTorchDetector:
    ...
    def detect(self, frame, classes):
        ...
        # BGR HWC -> RGB CHW。一次连续化后直接创建共享CPU Tensor，
        # 再用单次传输送入GPU。
    ...
    with mss.mss() as capture:
        ...
        while running:
            ...
            if debug_enabled:
                ...
                # waitKey(1)在部分Windows环境会实际阻塞约15.6 ms，
                # 与推理耗时叠加后很容易把循环压到约30 FPS。
                # pollKey只处理窗口消息，不额外等待。
                key = poll_window_key()
            ...
            # Windows sleep每次多睡的一点时间会不断累积，使60 FPS
            # 上限长期只能得到57～59 FPS。固定时间轴会在下一帧自动
