...
def build_person_candidates(detections, allowed_classes):
    """
    将同一人物的躯干框和头部框合并为一个候选目标。

    优先使用躯干框作为人物锚点；只有检测到头部、没有匹配到对应
    躯干时，才使用头部框作为候选，避免同一个人在候选列表里出现两次。
    """

    allowed_detections = [
        detection
        for detection in detections
        if detection["class_id"] in allowed_classes
    ]

    candidates = []

    # 有躯干框时，以躯干框代表这个人物。
    for detection in allowed_detections:
        team = get_team_from_class(detection["class_id"])
    ...
    for detection in allowed_detections:
        ...
        matched_body = find_body_for_head(
            detection,
            allowed_detections,
            body_class,
        )
...
class StickyTargetLock:
    """
    右键按住期间保持第一次选择的目标。

    原目标暂时漏检时停止移动，不会自动切换到其他人。
    松开右键后才解除锁定。
    """
    ...
    def acquire(
        ...
        def distance_to_crosshair(detection):
        ...
        # ByteTrack可能在新目标出现的第一帧还没有分配ID；
        # 此时继续等待下一帧，避免建立一个无法持续追踪的锁定。
        if not tracked_candidates:
            return None

        # 只在本轮右键刚建立锁定时执行一次：
        # 以实际锁点到画面中心准星的欧氏距离选择最近的人。
        selected = min(
            tracked_candidates,
            key=distance_to_crosshair,
        )

        self.acquired = True
        self.track_id = selected["track_id"]
        self.class_id = selected["class_id"]
        self.last_detection = selected.copy()
    ...
    def update(
        ...
        if self.track_id is not None:
            for detection in candidates:
                if (
                    ...
                    and detection["class_id"] == self.class_id
                ):
        ...
        for detection in same_class_candidates:
            current_iou = self.box_iou(
                self.last_detection,
                detection,
            )
        ...
        if best_match is None or best_iou < 0.25:
            return None

        self.last_detection = best_match.copy()

        if best_match["track_id"] is not None:
            self.track_id = best_match["track_id"]
...
def main():
    ...
    with mss.mss() as capture:
        ...
        while running:
            ...
            just_pressed = aiming and not previous_aiming
            just_released = not aiming and previous_aiming
