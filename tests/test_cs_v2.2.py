...
class AimController:
    ...
    def _robust_velocity(
        ...
        # 速度只用于抵消流水线延迟，必须比位置平滑更快。较短时间
        # 常数能在2～3帧内跟上横移；一致性门会过滤单帧框抖动。
        time_constant = 0.010 + smoothing * 0.012
        alpha = 1.0 - math.exp(
            -dt / max(0.001, time_constant)
        )

        # 一旦最近样本明确反向，立即释放旧方向速度，防止目标停下
        # 或变向后仍继续前馈。
        if (
            previous_velocity != 0.0
            and median_velocity * previous_velocity < 0.0
        ):
