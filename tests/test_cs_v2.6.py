...
# Interception 鼠标输入后端
# ============================================================
# 用于替换原有 user32.SendInput 调用。
# 需要安装对应 interception Python 包及驱动。
try:
    import interception
except ImportError:
    interception = None
...
DEFAULT_MODEL = Path(
    r"E:\Floder\VsCodeFile\PyCodes\yolo_enemy_detector\runs\yolo12_l\yolo12l_.engine"
)
...
class AimController:
    ...
    4. 把屏幕像素误差换算为游戏实际需要的鼠标单位；
    5. 按真实帧间隔归一化增益和最大步长，低帧率时不会自动变慢；
    6. 连续运动目标即使进入死区也保持受控维持量，消除固定滞后；
    7. 静止、停下或反向时立即撤销前馈并硬制动；
    8. 近锁点采用无惯性制动和最小连续脉冲，兼顾稳定与丝滑。
    """

    COMMAND_HISTORY_SECONDS = 0.30
    # 从SendInput到游戏画面真正反映鼠标移动，通常至少跨越一个
    # 渲染/采集间隔。取10 ms会略偏保守，但能够把尚未出现在当前
    # 截图里的旧指令计入“在途量”，避免靠近锁点后重复加速。
    COMMAND_VISIBILITY_DELAY = 0.010
    # 动态目标需要覆盖“截图年龄 + 推理 + 下一帧等待 + 输入生效”
    # 的总延迟。旧值30 ms且最多只补10 px，会让连续横移目标天然
    # 保留一段稳定滞后。这里允许更长的受控补偿，但实际量仍由
    # 运动一致性、目标尺度和当前位置共同限制。
    ...
    def __init__(self):
        ...
        # 速度窗口保持较短，避免目标已经加速、减速或停下后，中位数
        # 仍被很久以前的样本拖住。稳态抗抖由一致性门负责。
    ...
    def _fit_response(samples, current_response):
        ...
        # 不同游戏灵敏度、DPI和系统设置下，1个SendInput单位对应的
        # 屏幕位移可能远小于0.25 px。旧下限会在低灵敏度场景持续
        # 低估所需鼠标量，使XY拉满仍追不上。
        if not 0.08 <= estimated_response <= 6.00:
            return current_response

        # 采用适中的融合速度，约十余个有效样本即可接近真实响应；
        # 回归本身包含目标速度截距，不会把匀速横移简单当成灵敏度。
        return (
    ...
    def _robust_velocity(
        ...
        # 将0～1映射成完整、单调且肉眼可辨的平滑曲线。默认0.20
        # 仍接近原版手感；越靠近1.00，速度估计的去噪越明显。
        smooth_curve = (
            smoothing
            * smoothing
            * (3.0 - 2.0 * smoothing)
    ...
    def _motion_confidence(samples, velocity):
        """
        判断当前速度是否来自连续运动，而非检测框单帧抖动。

        至少需要3个样本；方向一致且中位绝对偏差较小时，才允许
        速度参与短时跟随补偿。最后一个样本承担快速刹车门：
        静止或突然反向时一帧内撤销前馈。
        """
    ...
    def _smooth_output(requested, previous, smoothing):
        ...
        # 使用完整0～1平滑曲线：0完全直出，0.25轻度平滑，
        # 0.50明显柔化，0.75强平滑，1.00最柔顺。默认0.20
        # 的新指令权重约0.91，保持本基线原有的较好手感。
        smooth_curve = (
            smoothing
            * smoothing
            * (3.0 - 2.0 * smoothing)
    ...
    def update(
        ...
        """
        使用响应自适应、帧率无关的闭环控制把当前实测锁点拉向准星。

        当前框中心始终是位置闭环的唯一目标。速度只在多个连续样本
        方向一致时补偿截图与输入延迟，并且不能把指令推离当前锁点。
        输出降速或反向时立即响应，不携带旧方向惯性。
        """
        now = time.perf_counter()
        ...
        # 先判断目标是否真的在连续运动。旧版在误差进入死区后无条件
        # 停止，即使敌人仍匀速横移也要等下一帧落后后再追，形成
        # “紫点始终在前、准星一顿一顿跟随”的固定滞后。
        confidence_x = (
        ...
        # 静态目标进入锁定区后仍然硬制动并清空全部余量；只有经过
        # 连续样本确认的动态目标，才允许在死区内保持速度前馈。
        ...
        # 径向软死区：静态位置纠偏仍共享同一个半径比例。动态目标
        # 位于死区内时位置项为0，只保留可信速度的维持量。
        ...
        # 以120 FPS为强度参数的参考帧率，再按真实截图间隔换算。
        # 因此推理只有60 FPS时，每帧会自动承担约2倍的追赶量，
        # 不会出现同一XY值随帧率下降而明显变慢的情况。
        ...
        # 近中心仍连续降速，但不再把请求压到极小后隔几帧才凑出
        # 1个整数鼠标单位。实际是否需要发送1单位由后面的在途输入
        # 补偿和安全量化共同决定。
        brake_radius = clamp(
            target_scale * 0.16,
            7.0,
        ...
        if not measurement_valid:
            # 丢框时预测点只用于保持目标身份，不再发送鼠标。下一帧
            # 当前框重新出现后直接按其实测中心恢复闭环。
            position_screen_x = 0.0
            position_screen_y = 0.0

        # 对连续运动补偿完整流水线延迟。它使用扣除了自身鼠标输入
        # 后的目标速度，因此不会把正在生效的旧指令重复计算。
        ...
        # 强度0～3同时作用于动态维持量。旧版在这里把强度截到1，
        # 导致1.0、1.8、3.0对横移目标几乎没有区别。
        lead_gain_x = clamp(
        ...
        # 当误差与目标速度同向时，说明准星确实落在运动目标后方。
        # 追加一小段“消除现有滞后”的追赶量；一旦准星领先、目标
        # 停止或变向，该项立即为0，不会形成旧方向惯性。
        if (
            confidence_x > 0.0
        ...
        # 方向保护基于“当前误差 + 可信动态补偿”形成的即时引导点。
        # 旧版只看当前误差，会在准星刚到紫点时把维持量删掉；新版
        # 允许沿目标运动方向持续跟随，同时仍禁止绕着引导点运动。
        ...
        # response表示1个逻辑鼠标单位实际改变多少屏幕像素。旧版只
        # 估算它，却没有用于输出换算，这是低游戏灵敏度下XY=3.0
        # 依旧追不上的主要原因。
        ...
        # 即时引导误差换向表示控制方向确实改变。动态目标穿过准星时
        # 不会因为“当前误差”短暂换向而误刹车；目标真正停下或反向
        # 时仍会立即清除旧方向余量。
        if (
            self.previous_control_error_x != 0.0
            and guidance_x
        ...
        # 平滑只限制同方向的突然加速；需求减小、停止或换向立即
        # 跟随当前值，所以不会产生拖尾、回弹或行星式绕圈。
        ...
        # max-step按120 FPS下的单帧量解释，并随真实帧间隔放大。
        # 最大只放大4倍，既补偿低帧率，也避免一次发送异常巨量。
        ...
        # SendInput只能发送整数。旧版在0～1之间累计数帧后才发一次，
        # 视觉上就是轻微“走一下、停一下”。当轴向误差仍明确大于
        # 死区、且本帧需求至少0.30单位时，改为连续发送最小1单位。
        # 在途输入会在下一帧提前扣除，因此接近中心后不会多发。
        minimum_pulse = 0.30
        ...
        # 浮点向量在转换为两个整数轴后，极少数斜向小指令可能因
        # 舍入改变整体方向。发送前再次检查屏幕空间点积；如果整数
        # 合量反而远离即时引导点，只保留仍能靠近目标的那个轴。
        screen_dot_x = (
            move_x
