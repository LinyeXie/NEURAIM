...
class AimController:
    ...
    def update(
        ...
        """
        使用无惯性的径向闭环控制把当前实测锁点拉向准星。

        目标速度和历史鼠标量仍用于目标关联及在途指令补偿，但不会
        直接叠加到最终鼠标输出。这样静止或突然停下的目标附近不会
        残留切向速度，也就不会绕着中心回弹。
        """
        now = time.perf_counter()
        ...
        # 当前框中心一旦进入锁定区，立即硬制动。这里不仅停止本帧，
        # 还清空滤波输出和不足1像素的累计余量，旧指令不能在后续
        # 帧重新凑成一次移动。
        ...
        # 径向软死区：X/Y共享同一个半径比例，输出方向始终指向
        # 当前锁点，不会因为两轴分别过死区而产生切向“绕圈”。
        radial_scale = (
        ...
        # 在人物框尺度的约18%范围内连续制动。距离越近，增益越低；
        # 远处仍保持原来的追踪力度。smooth只改变阻尼强度，不再
        # 保存上一帧输出，因此不会产生平滑惯性。
        brake_radius = clamp(
            target_scale * 0.18,
            8.0,
        ...
        # 实测误差换向表示已经越过该轴中心。立刻丢弃旧方向的小数
        # 余量；本帧从新的小修正重新开始，不再等待两帧反向确认。
        if (
            self.previous_control_error_x != 0.0
            and predicted_error_x
            * self.previous_control_error_x <= 0.0
        ):
            self.residual_x = 0.0
            requested_x *= 0.35
        ...
        if requested_length > max_step > 0.0:
            vector_scale = max_step / requested_length
